...
def trigger_webhook(peak: float, rms: float, gap_ms: int) -> None:
    ...
    while True:
        ...
        if impulse:
            ...
            if first_clap is None:
                first_clap = now
                print(
                    f"1. Impuls: peak={peak:.3f}, "
                    f"rms={rms:.3f}, baseline={baseline:.3f}"
                )
                continue
            ...
            if min_gap <= gap <= max_gap:
                try:
                    trigger_webhook(peak, rms, gap_ms)
                    last_event = now
